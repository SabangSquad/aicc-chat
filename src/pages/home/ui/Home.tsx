...
export function Home() {
  ...
  const sendMessage = async (e?: React.FormEvent, keyword?: string) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "bot", content: "서버와 연결할 수 없습니다. 잠시 후 다시 시도해주세요." },
      ]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="relative flex flex-col h-screen font-sans overflow-hidden bg-white text-slate-900 selection:bg-[#e8499f] selection:text-white">
      ...
      <style jsx>{`
        ...
        .animate-wave { animation: loading-wave 1.2s ease-in-out infinite; }
      ...
      <main className="flex-1 w-full max-w-3xl mx-auto p-4 overflow-y-auto z-10 scrollbar-hide">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 text-center gap-6 animate-fade-in-up">
            <div className={`w-20 h-20 rounded-3xl flex items-center justify-center shadow-2xl shadow-[#ac55f2]/20 mb-2 ${BRAND_GRADIENT}`}>
            ...
            <p className="text-slate-500 max-w-md leading-relaxed">
              궁금한 내용을 자유롭게 물어보세요.<br/>AI가 친절하게 답변해 드립니다.
            </p>
          </div>
        )}
        ...
        {messages.map((msg, index) => (
          ...
          >
            {msg.role === "bot" && (
              <div className={`flex-shrink-0 w-9 h-9 rounded-full flex items-center justify-center shadow-md ${BRAND_GRADIENT}`}>
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="px-6 py-5 bg-slate-50 border border-slate-100 rounded-2xl rounded-bl-none shadow-sm">
                <div className="flex gap-2 items-center h-full">
                  <span className="w-2.5 h-2.5 rounded-full bg-[#ac55f2] animate-wave" style={{ animationDelay: '0ms' }}></span>
                  <span className="w-2.5 h-2.5 rounded-full bg-[#e8499f] animate-wave" style={{ animationDelay: '200ms' }}></span>
                  <span className="w-2.5 h-2.5 rounded-full bg-[#ff6b6b] animate-wave" style={{ animationDelay: '400ms' }}></span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <form onSubmit={(e) => sendMessage(e)} className="relative flex items-center group">
            ...
            <button
              ...
              className={`absolute right-2 p-2.5 rounded-full text-white transition-all shadow-md hover:shadow-lg hover:scale-105 disabled:opacity-50 disabled:hover:scale-100 disabled:cursor-not-allowed ${BRAND_GRADIENT}`}
            >
              {isLoading ? (
